                     ...
                     <form class="w-100 validateForm" style= "color:red;">
                        ...
                        <div class="col-md-12 pro-form dis-ver-center p-0">
                           <div class="col-md-6 col-sm-12">
                              ...
                              <input class="form-control" type="email" id ="email" name="email" placeholder="@lang('user.profile.email')">
                           </div>
                           <div class="col-md-6 col-sm-12">
                        ...
                        <div class="col-md-12 pro-form dis-ver-center p-0">
                           ...
                           <div class="col-md-4 col-sm-12">
                              <h5 class=""><strong>State</strong></h5>
                              <select id="state" name="state_id" @if(Helper::getDemomode() == 1) disabled="disabled" @endif  class=" mb-4 form-control">
                        ...
                        <div class="col-md-12 pro-form dis-ver-center p-0">
                           ...
                           <div class="col-md-6 col-sm-12">
                              <h5 class=""><strong>Licence Number</strong></h5>
                              <input class="form-control" type="text" id ="licence_number" name="licence_number" placeholder="Licence Number">
                           </div>
                        </div>
                        
                        <div class="col-md-12 pro-form dis-ver-center p-0">
                           <div class="col-md-6 col-sm-12">
                              <h5 class=""><strong>Licence Front</strong></h5>
                              <div class="licence-section">
                                 <img class = "licence_front-img"/>
                                 <div class="fileUpload up-btn profile-up-btn">
                                    <span class="upload-text">Upload Licence Front</span>
                                    <input type="file" id="licence_front_img_upload_btn" name="licence_front" class="upload" accept="image/x-png, image/jpeg">
                                 </div>
                              </div>
                           </div>
                           <div class="col-md-6 col-sm-12">
                              <h5 class=""><strong>Licence Back</strong></h5>
                              <div class="licence-section">
                                 <img class ="licence_back-img" />
                                 <div class="fileUpload up-btn profile-up-btn">
                                    <span class="upload-text">Upload Licence Back</span>
                                    <input type="file" id="licence_back_img_upload_btn" name="licence_back" class="upload" accept="image/x-png, image/jpeg">
                                 </div>
                              </div>
                           </div>
                        </div>
...
var front = '';
var back = '';
$('#licence_front_img_upload_btn').on('change', function(e) {
      var files = e.target.files;
      if (files && files.length > 0) {
         licence_front = files[0].name;
         cropImage($(this), files[0], $('.licence_front-img'), function(data) {
            front = data;
         });
      }
   });

$('#licence_back_img_upload_btn').on('change', function(e) {
      var files = e.target.files;
      if (files && files.length > 0) {
         licence_back = files[0].name;
         cropImage($(this), files[0], $('.licence_back-img'), function(data) {
            back = data;
         });
      }
   });
...
function getCities(state_id, cityname){
   $.ajax({
        type:"GET",
        url: getBaseUrl() + "/user/citybystate/"+state_id,
        'beforeSend': function (request) {
            showLoader();
        },
        success:function(data){
	 ...
	 $.ajax({
        ...
        success:function(response){
         ...
         $('.account_balance_wallet').text(result.country.country_symbol+' '+result.wallet_balance);
         $('.user-img').attr('src',result.picture);
         $('.licence_front-img').attr('src',result.licence_front);
         $('.licence_back-img').attr('src',result.licence_back);
         $('select[name=country_id]').val(result.country_id).trigger('change');

         $('#state option').each(function(){
            var stateid = $(this).val();
            if(stateid==result.state_id){
               $(this).prop('selected', true);
            }
         });

         getCities(result.state_id, result.city.city_name);
   ...
   $('#first_name').keyup(function(){
      if($('#first_name').val()!=''){
         $('#first_name').parents('.col-sm-12').find('.error').remove();
      }
   });
   $('#last_name').keyup(function(){
      if($('#last_name').val()!=''){
         $('#last_name').parents('.col-sm-12').find('.error').remove();
      }
   });
   $('#mobile').keyup(function(){
      if($('#mobile').val()!=''){
         $('#mobile').parents('.col-sm-12').find('.error').remove();
      }
   });
   $('#email').keyup(function(){
      if($('#email').val()!=''){
         $('#email').parents('.col-sm-12').find('.error').remove();
      }
   });
   $('#city').change(function(){
      if($('#city').val()!=''){
         $('#city').parents('.col-sm-12').find('.error').remove();
      }
   });
   $('#state').change(function(){
      if($('#state').val()!=''){
         $('#state').parents('.col-sm-12').find('.error').remove();
      }
   });
    ...
    //For profile details
     $('.validateForm').validate({
		...
		messages: {
         ...
         city: { required: "{{__('auth.city_required')}}" },
         state: { required: "{{__('auth.state_required')}}" },
         licence_number: { required: "{{__('auth.licence_number')}}" },

		},
		highlight: function(element)
		{
			$(element).closest('.col-sm-12').addClass('has-error');
		},
		success: function(label) {
			label.closest('.col-sm-12').removeClass('has-error');
			label.remove();
		},
		submitHandler: function(form) {
         ...
         var error=0;
         if(fname==''){
            $('#first_name').after('<label class="error">Please enter first name</label>');
            error++;
         }
         if(lname==''){
            $('#last_name').after('<label class="error">Please enter last name</label>');
            error++;
         }
         if(email==''){
            $('#email').after('<label class="error">Please enter email</label>');
            error++;
         }
         if(mobile==''){
            $('#mobile').after('<label class="error">Please enter mobile</label>');
            error++;
         }
         if(city==''){
            $('#city').after('<label class="error">Please select city</label>');
            error++;
         }
         if(state==''){
            $('#state').after('<label class="error">Please select state</label>');
            error++;
         }
         ...
         if(error>0){
            ...
            if(front != "") data.append('licence_front', front, licence_front);
            if(back != "") data.append('licence_back', back, licence_back);
            ...
            $.ajax({
                     ...
                     error: function(jqXHR, textStatus, errorThrown) {
                           ...
                           if (data.message) {
                              alertMessage(textStatus, data.message, "danger");
                           }
                           hideInlineLoader();
                     }
                  });
