...
function getToken(guard) {
    if (guard == "admin") {
        ...
        if (localStorage.ownerAccess != null) {
            return localStorage.ownerAccess;
        }
    }
...
function setOwnerDetails(data) {
    localStorage.setItem("owner", JSON.stringify(data));
}

function getOwnerDetails() {
    if (localStorage.owner != null) {
        return JSON.parse(localStorage.owner);
    }
...
var params = 'url='+window.location.origin+'&key=607998047ee83';
