    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(data.responseData, function(key, country) {
                var newOption = new Option(country.country.country_name, country.country.id, false, false);
                $('#country_list').append(newOption).trigger('change');

                if (country.company_country_states) {
                    state[country.country.id] = country.company_country_states
                }
                if (country.company_country_cities) {
                    city[country.country.id] = country.company_country_cities
                }
            });
...
$('body').on('change', '#country_list', function() {
    ...
    $.each(state[value], function(key, stateData) {
        var newOption = new Option(stateData.state.state_name, stateData.state.id, false, false);
        $('#state_list').html(newOption).trigger('change');
    });

    $.each(city[value], function(key, cityData) {
        var newOption = new Option(cityData.city.city_name, cityData.city.id, false, false);
        $('#city_list').html(newOption).trigger('change');
    });
