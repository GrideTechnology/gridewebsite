  ...
  $.ajax({
        ...
        success: function(response, textStatus, jqXHR) {
            ...
            if(page!=undefined){
                ...
                setTimeout(function(){
                    window.location.replace(window.url+page);
                  }, 1000);
            }
        },
        error: function(jqXHR, textStatus, errorThrown) {
...
function setData(url, guard = "admin") {
    ...
    $.ajax({
        ...
        success: function(response, textStatus, jqXHR) {
            ...
            if(data.provider_document_url){
                $("#document_name").html(data.provider_document_url.substring(data.provider_document_url.lastIndexOf("/") + 1, data.provider_document_url.length));
            }
            ...
            if(data.time){
                var yourDate = new Date(data.time);
                const offset = yourDate.getTimezoneOffset()
                yourDate = new Date(yourDate.getTime() - (offset*60*1000))
                yourDate = yourDate.toISOString().split('T')[0];
                $('#publish_date').val(yourDate);
            }
