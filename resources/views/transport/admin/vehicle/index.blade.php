                ...
                <div class="card card-small">
                    <div class="card-header border-bottom">
                        ...
                        <a href="javascript:;" class="btn btn-success pull-right add"><i class="fa fa-plus"></i>
                            {{ __('transport.admin.vehicletype.add') }}</a>
...
@section('scripts')
    ...
    console.log(CryptoJSAesEncrypt('FbcCY2yCFBwVCUE9R+6kJ4fAL4BJxxjd', 'Hello World'));*/
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            setTimeout(() => {
                                lightGallery(document.getElementById('vehicle_image' + row
                                    .id), {
                                    download: false,
                                    share: false,
                                });
                            }, 1000);
                            return '<ul class="small-block-grid-2 medium-block-grid-3" id="vehicle_image' +
                                row.id + '" ><li data-src="' + data + '"> <a href><img src="' +
                                data + '" style="height: 50px;"></a></li></ul>';
                        }
                    },
                    {
                        "data": "vehicle_marker",
                        render: function(data, type, row) {
                            setTimeout(() => {
                                lightGallery(document.getElementById('vehicle_marker' + row
                                    .id), {
                                    download: false,
                                    share: false,
                                });
                            }, 1000);
                            return '<ul class="small-block-grid-2 medium-block-grid-3" id="vehicle_marker' +
                                row.id + '" ><li data-src="' + data + '"> <a href><img src="' +
                                data + '" style="height: 50px;"></a></li></ul>';
                        }
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            if (data == null) {
                                ...
                                return data.ride_name !== null ? data.ride_name : '';
                            }
                        }
                    },
                    {
                    ...
                    <li><a href="javascript:;" data-toggle="modal" data-target="#service_type_2" onclick="getPrice(` +
                                    data.id + `)" class="dropdown-item price"><i class="fa fa-money"></i> Price</a> </li>
                    ...
                    <li><a href="javascript:;" data-id="` + data.id + `" data-value="` + data.status +
                                    `" data-sab_url="/admin/vehicle/` + data.id +
                                    `" class="dropdown-item delete-record"><i class="fa fa-trash"></i>&nbsp;Delete</a> </li> </ul> </li></div>`;
                                return button;
                            } else {
            ...
            $('body').on('click', '.status', function() {
                ...
                $(".status-modal-btn")
                    ...
                    .on("click", function() {
                        ...
                        var url = getBaseUrl() + "/admin/vehicle/" + id + '/updateStatus?status=' +
                            value;
        ...
        $('body').on('change', '#country_list', function() {
            ...
            //     // $('#state_list').append(`<option value="${stateData.id}">
            ...
            //     // $('#city_list').append(`<option value="${cityData.id}">
            // //                     ${cityData.city_name}
            // //                 </option>`);
            // });
        ...
        $('body').on('change', '#city_list', function() {
            ...
            if (value && country_id && state_id) {
                getData(price_id, value, country_id, null ,state_id)
            }
        ...
        function getPrice(id) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    //         countryCityList += `<label class="country_list_style">` + country
                    //             .country_name +
                    //             `<span class="pull-right"><i class="fa fa-files-o fa-lg" aria-hidden="true"></i></span></label>`;
                    //         $.each(country.city, function(key1, city) {
                    //             if (city != null) {
                    //                 if (key == 0 && key1 == 0) {
                    //                     var cityActiveClass = "active";
                    //                     console.log(city);
                    //                     getData(id, city.id, country.id, country.currency,null);
                    ...
                    //                 countryCityList +=
                    //                     `<a href="javascript:;" class="list-group-item cityActiveClass  ` +
                    //                     cityActiveClass + `" data-value="` + country.currency +
                    //                     `" onclick="getData(` + id + `,` + city.id + `,` +
                    //                     country.id + `)" id="` + city.id + `"><span>` + city
                    //                     .city_name + `</span></a>`;
                    //             }
                    //         });
                    //     }
        ...
        function getData(vehicleId, cityId, countryId, currency_value, stateId) {
            ...
            var service_fee = 0;
            var airport_fee = 0;
            var scheduled_cancellation_fee = 0;
            var scheduled_minimum_fee = 0;
            var distance_type = 1;
            var tax = 0;
            var priceList = [];
            var url = getBaseUrl() + "/admin/rideprice/" + vehicleId + "/" + stateId+ "/" + cityId;
            $.ajax({
                url: url,
                type: "GET",
                async: false,
                ...
                success: function(data) {
                    ...
                    if (data.responseData.price.length != 0) {
                        ...
                        tax = priceData.tax;
                        default_price = priceData.default_price;
                        cancellation_fee = priceData.cancellation_fee;
                        max_price = priceData.max_price;
                        min_price = priceData.min_price;
                        service_fee = priceData.service_fee;
                        airport_fee = priceData.airport_fee;
                        scheduled_cancellation_fee = priceData.scheduled_cancellation_fee;
                        scheduled_minimum_fee = priceData.scheduled_minimum_fee;
                        distance_type = priceData.distance_type;
                    }
                    ...
                    $.each(data.responseData.peakHour, function(key, value) {
                        ...
                        peak += `<tr>
                                            ...
                                            <td>
                                            <input type="hidden" id="peak_price_id" name="peak_price[` + value.id +
                            `][id]" value="` + peak_price_id + `" min="1">
                                                <input type="text" id="peak_price" name="peak_price[` + value.id +
                            `][value]" value="` + peak_price + `" min="1">
                                            </td>
                                        </tr>`;
                    });

                    console.log("geofence",data.responseData.geofence);
                    $.each(data.responseData.geofence, function(key, value) {
                        var price_list = priceList.find(price => price.geofence_id == value.id);
                        geofence +=
                            `<tr><td><label style="float:left; margin-bottom: 0; text-style: bold;" for='` +
                            value.id + `'>
                        ...
                        if (data.responseData.geofence.length > 1) {
                            ...
                            if (price_list != undefined && price_list.pricing_differs == 1) {
                                ...
                                var default_price = price_list.default_price;
                                var cancellation_fee = price_list.cancellation_fee;
                                var max_price = price_list.max_price;
                                var min_price = price_list.min_price;
                                var service_fee = price_list.service_fee;
                                var airport_fee = price_list.airport_fee;
                                var scheduled_cancellation_fee = price_list.scheduled_cancellation_fee;
                                var scheduled_minimum_fee = price_list.scheduled_minimum_fee;
                                var distance_type = price_list.distance_type;
                                var current_peak = '';
                                ...
                                $.each(price_list.ridePeakhour, function(key, value) {
                                    ...
                                    current_peak += `<tr>
                                                        ...
                                                        <td>` + value.started_time + ` - ` + value.ended_time + `</td>
                                                        <td>
                                                        <input type="hidden" id="peak_price_id" name="peak_price[` +
                                        value.id + `][id]" value="` + value.id + `" min="1">
                                                            <input type="text" id="peak_price" name="peak_price[` +
                                        value.id + `][value]" value="` + value.peak_price + `" min="1">
                                                        </td>
                                                    </tr>`;
                                });


                                formContentText = formContent(current_calculator, current_hour,
                                    current_fixed, current_distance, current_minute, current_price,
                                    current_commission, current_fleet_commission, current_tax,
                                    current_peak_commission, current_waiting_commission,
                                    current_peak, current_waiting_free_mins,
                                    current_waiting_min_charge, default_price, cancellation_fee, max_price, min_price, service_fee, airport_fee, scheduled_cancellation_fee, scheduled_minimum_fee, distance_type);
                            }

                            geofence += ` class="different_pricing" value='` + value.id +
            ...
            function formContent(calculator = 'DISTANCE', hour = '', fixed = '', distance = '', minute = '', price = '',
                commission = 0, fleet_commission = 0, tax = 0, peak_commission = 0, waiting_commission = 0, peak = '',
                waiting_free_mins = '', waiting_min_charge = '' , default_price = 0, cancellation_fee = 0, max_price = 0, min_price = 0, service_fee = 0, airport_fee = 0, scheduled_cancellation_fee = 0, scheduled_minimum_fee = 0, distance_type = 1) {
                distance_text = distance_type == 1 ? 'Kms' : 'Miles';
                ...
                return `<!-- Pricing for Country -->
                                <div class="form-row">
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input class="form-control" type="number" name="default_price" id="default" placeholder="Default" min="0" value="` + default_price + `">
                                        <span class="txt_clr_4"><i>Base price of the ride</i></span>
                                    </div>

                                    <div class="form-group col-md-6">
                                        <label for="cancellation_fees">Cancellation fees</label>
                                        <input class="form-control" type="number" value="` + cancellation_fee + `" name="cancellation_fee" id="cancellation_fees" placeholder="Cancellation fees" min="0">
                                        <span class="txt_clr_4"><i>Cancellation fees of the ride</i></span>
                                    </div>

                                    <div class="form-group col-md-6">
                                        <label for="minimum_for_trips">Minimum amount for trips</label>
                                        <input class="form-control" type="number" value="` + min_price + `" name="min_price" id="minimum_for_trips" placeholder="Minimum amount for trips" min="0">
                                        <span class="txt_clr_4"><i>Minimum amount of the ride</i></span>
                                    </div>

                                    <div class="form-group col-md-6">
                                        <label for="maximum_for_trips">Maximum amount for trips</label>
                                        <input class="form-control" type="number" value="` + max_price + `" name="max_price" id="maximum_for_trips" placeholder="Maximum amount for trips" min="0">
                                        <span class="txt_clr_4"><i>Maximum amount of the ride</i></span>
                                    </div>

                                    <div class="form-group col-md-6">
                                        <label for="service_fee">Service fees</label>
                                        <input class="form-control" type="number" value="` + service_fee + `" name="service_fee" id="service_fee" placeholder="Service fees" min="0">
                                        <span class="txt_clr_4"><i>service fees of the ride</i></span>
                                    </div>

                                    <div class="form-group col-md-6">
                                        <label for="airport_price_fee">Airport price/fee</label>
                                        <input class="form-control" type="number" value="` + airport_fee + `" name="airport_fee" id="airport_price_fee" placeholder="Airport price/fee" min="0">
                                        <span class="txt_clr_4"><i>Airport price/fee of the ride</i></span>
                                    </div>

                                    <div class="form-group col-md-6">
                                        <label for="scheduled_cancellation_fee">Scheduled cancellation fees/fee</label>
                                        <input class="form-control" type="number" value="` + scheduled_cancellation_fee + `" name="scheduled_cancellation_fee" id="scheduled_cancellation_fee" placeholder="Scheduled cancellation fees" min="0">
                                        <span class="txt_clr_4"><i>Scheduled cancellation fees of the ride</i></span>
                                    </div>

                                    <div class="form-group col-md-6">
                                        <label for="scheduled_minimum_fee">Scheduled minimum fee</label>
                                        <input class="form-control" type="number" value="` + scheduled_minimum_fee + `" name="scheduled_minimum_fee" id="scheduled_minimum_fee" placeholder="Scheduled minimum fee" min="0">
                                        <span class="txt_clr_4"><i>Scheduled minimum fee of the ride</i></span>
                                    </div>
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <select class="form-control" name="calculator" id="calculator">
                                            ...
                                            <option value="DISTANCE"  ` + (calculator == "DISTANCE" ? 'selected' :
                    '') + `>Distance Pricing</option>
                                            <option value="DISTANCEMIN"  ` + (calculator == "DISTANCEMIN" ?
                        'selected' : '') + `>Distance and Per Minute Pricing</option>
                                            <option value="DISTANCEHOUR"  ` + (calculator == "DISTANCEHOUR" ?
                        'selected' : '') + `>Distance and Per Hour Pricing</option>
                                        </select>
                                    ...
                                    <div class="form-group col-md-6">
                                        <label for="feFirstName" id="base_distance">Base Distance (0 `+ distance_text +`)</label>
                                    ...
                                    <div class="form-group col-md-6">
                                        <label for="feFirstName" id="unit_distance">Unit Distance Pricing (0 Miles)</label>
                                    ...
                                    <input type="hidden" name="distance_type" id="distance_type" value="2"/>
                                    <!--<div class="form-group col-md-6 allowdiv">
                                        <label for="customToggle2"> {{ __('Distance type') }}</label>
                                        <select class="form-control" name="distance_type" id="distance_type">
                                            <option value="1"  ` + (distance_type == "1" ? 'selected' : '') + ` >KMs</option>
                                            <option value="2"  ` + (distance_type == "2" ? 'selected' : '') + `>Miles</option>
                                        </select>
                                    </div> -->

                                    <div class="form-group col-md-6">
                                        <label for="feFirstName">Commission(%)</label>
                                        <input class="form-control decimal" type="text" value="` + (commission ?
                        commission : 0) + `" name="commission" id="commission" placeholder="Commission" min="0">
                                    </div>
                                    <div class="form-group col-md-6">
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input class="form-control numbers" type="number" value="` +
                    waiting_free_mins + `" name="waiting_free_mins" id="waiting_free_mins" placeholder="Waive off minutes" min="0">
                                    </div>
                                    <div class="form-group col-md-6">
                    ...
                    <div class="col-xs-12">
                        ...
                        <div class="tab-content pricing-nav nav-wrapper" id="nav-tabContent">
                            ...
                            <input type="hidden" value="` + countryId + `"name="country_id">
                                ...
                                ` + formContent(calculator, hour, fixed, distance, minute, price, commission,
                fleet_commission, tax, peak_commission, waiting_commission, peak, waiting_free_mins,
                waiting_min_charge, default_price, cancellation_fee, max_price, min_price, service_fee, airport_fee, scheduled_cancellation_fee, scheduled_minimum_fee, distance_type) + `
...
<table id="country_serv_type" class="table table-hover table_width display" style="width:100%">
                                        ...
                                        <div class="col-xs-10">
                                            <div class="row">
                                                <div class="col-xs-12 col-sm-6 col-md-4">
                                                    <a href="javascript:;" class="btn btn-danger btn-block addPrice">Submit</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
        ...
        $('body').on('change', '#distance_type', function(event) {
            if ($(this).val() == '2'){
                $('#base_distance').html('Base Distance (0 Miles)');
                $('#distance').val(roundOff($('#distance').val() / 1.609 , 2));
            }else{
                $('#base_distance').html('Base Distance (0 Kms)');
                $('#distance').val(roundOff($('#distance').val() * 1.609 , 2));
            }
        });
        ...
        function priceInputs(cal) {
            if (cal == 'MIN') {
                ...
                $("#changecal").text(
                    'BP + (T{{ __('transport.admin.vehicletype.distance') }}-BD*P{{ __('transport.admin.vehicletype.distance') }})'
                    );
                ...
                $("#changecal").text(
                    'BP + (T{{ __('transport.admin.vehicletype.distance') }}-BD*P{{ __('transport.admin.vehicletype.distance') }}) + (TM*PM)'
                    );
                ...
                $("#changecal").text(
                    'BP + ((T{{ __('transport.admin.vehicletype.distance') }}-BD)*P{{ __('transport.admin.vehicletype.distance') }}) + (TH*PH)'
                    );
            } else {
                $("#minute,#hourly_price").attr('value', '');
                $("#price,#distance").prop('disabled', false);
