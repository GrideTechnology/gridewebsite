...
div#dashboard-home .card .row .flex-grow-0.border-top .button{
						...
						<div class="col-12 col-lg-7 col-xl-9">
							...
							<div class="flex-grow-0 py-3 px-4 border-top">
								<div class="input-group">
									...
									<button type="button" class="btn btn-primary send" onclick="sendMessage()">Send</button>
								</div>
							</div>
...
$.ajax({
    url: getBaseUrl()+"/user/transport/user-inbox",
    type: "GET",
    headers: {
        Authorization: "Bearer " + getToken("user")
    },
    success: function(response){
    	var userList = response.responseData.chats;
    	userlistHtml=''
    	if(userList==''){
    		...
    		userList.forEach(function(item, index){  
		        ...
		        userlistHtml+='<a href="javascript:void(0)" onclick="getUserMessages('+item.owner_id+')" class="list-group-item list-group-item-action border-0"><div class="d-flex align-items-start"><img src="'+profile+'" class="rounded-circle mr-1" alt="'+item.first_name+' '+item.last_name+'" width="40" height="40"><div class="flex-grow-1 ml-3">'+item.first_name+' '+item.last_name+'</div></div></a>';
		    });
    	}        
...
function getUserMessages(id=''){
	...
	$.ajax({
	    url: getBaseUrl()+"/user/transport/user-messages",
	    type: "POST",
        data: {id: id},
	    headers: {
	        Authorization: "Bearer " + getToken("user")
	    ...
	    success: function(response){
	    	var ownerList = response.responseData.owner;
	    	var user = response.responseData.user;
	    	messageHtml='';
	    	if(userList==''){
	    		...
	    		ownerList.forEach(function(item, index){  
	    			...
	    			if(item.message!='notfound'){
    		    		...
    		    		if(item.from=='user'){
    		    			...
    		    			messageHtml+='<div class="chat-message-right pb-4"><div><img src="'+user.picture+'" class="rounded-circle mr-1" alt="'+user.first_name+' '+user.last_name+'" width="40" height="40"></div><div class="flex-shrink-1 bg-light rounded py-2 px-3 mr-3"><div class="font-weight-bold mb-1">You</div>'+item.message+'</div></div>';

                            $('.userimg').attr('src', ownerprofile);
                            $('.userimg').show();
...
function sendMessage(){
	...
	$.ajax({
	    url: getBaseUrl()+"/user/transport/send-user-message",
	    type: "POST",
	    data: {message: msg, owner: owner},
	    headers: {
	        Authorization: "Bearer " + getToken("user")
	    },
	    success: function(response){
