        ...
        <div class="col-md-12">
            <div class="card card-small">
                <div class="card-header border-bottom">
                    ...
                    <a href="javascript:;" class="btn btn-success pull-right add new_user"><i class="fa fa-plus"></i> {{ __('Add New') }} {{ __('Blog') }}</a>
                ...
                <thead>
                    <tr>
                        ...
                        <th data-value="first_name">{{ __('admin.title') }}</th>
                        <th data-value="last_name">{{ __('admin.date') }}</th>
...
<script>
    ...
    $(document).ready(function() {
        ...
        var url = getBaseUrl() + "/admin/blog";
        var keys = {id: "S.No.", title: "Title", date: "Publish Date"};

        datatable_export(url, keys, {{Helper::getEncrypt()}});

        $('.add').on('click', function(e) {
            e.preventDefault();
            $.get("{{ url('admin/blog/create') }}", function(data) {
        ...
        $('body').on('click', '.edit', function(e) {
            e.preventDefault();
            $.get("{{ url('admin/blog/') }}/"+$(this).data('id')+"/edit", function(data) {
                $('.crud-modal .modal-container').html("");
                $('.crud-modal .modal-container').html(data);
            });
            $('.crud-modal').modal('show');
        ...
        table = table.DataTable( {
            ...
            "columns": [
                ...
                { "data": function (data, type, row) {
                 ...
                 button +='<li><a href="javascript:;" data-id="'+data.id+'" data-value="'+data.id+'" data-sab_url="/admin/blog/'+data.id+'" class="dropdown-item delete-record"><i class="fa fa-trash"></i>&nbsp;Delete</a> </li>';
                  button +='</ul> </li></div>';
