...
class HomeController extends Controller
{
	...
	public function reviews(Request $request, $id){
		...
		if($request->has('limit')) {
			$review = Rating::select('id','admin_service','user_id','provider_id','provider_rating','provider_comment','user_comment','user_rating','created_at')->where('provider_id',$id)->where(['company_id'=>Auth::guard('provider')->user()->company_id])->whereNotNull('user_comment')->where('user_comment','!=',"")
			->with([
			...
			$review = Rating::select('id','admin_service','user_id','provider_id','provider_rating','provider_comment','user_comment','user_rating','created_at')->where('provider_id',$id)->where(['company_id'=>Auth::guard('provider')->user()->company_id])
			->with([
					'user' => function($query){  $query->select('id', 'first_name', 'last_name', 'picture' ); },
			])->orderby('id','desc')->get();
		}
	...
	public function addvechile(Request $request)
	{
		...
		try{
			if($request->admin_service != 'SERVICE'){
                ...
                // if($request->has('vin_number')) $providervehicle->vin_number=$request->vin_number;
	...
	public function editvechile(Request $request)
	{
		...
		try{
			if($request->admin_service !='SERVICE'){
				...
				// ffif($request->has('vin_number')) $providervehicle->vin_number=$request->vin_number;
				$providervehicle->vin_number=$request->vin_number;
			    if($request->hasFile('vechile_image')) $providervehicle->vechile_image = Helper::upload_file($request->file('vechile_image'), 'provider', null, Auth::guard('provider')->user()->company_id);
