...
class RideController extends Controller
{
	...
	{        
		...
		})->where('ride_type_id', $request->type)->where('company_id', $this->company_id)->whereIn('is_female',$is_female)->where('status', 1)->whereNotIn('id', $service_id_list)->select('*', \DB::raw('"..." AS "estimated_time"'))->get();
	    }
		else
		{
			$ride_delivery_vehicles = RideDeliveryVehicle::with(['priceDetails' => function($q) use($state_id) {
